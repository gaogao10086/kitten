...
public interface MenuMapper {
    ...
    List<MenuEntity> getMenuList() throws Exception;

    @SelectProvider(type = MenuProvider.class, method = "getActiveMenuList")
    List<MenuEntity> getActiveMenuList() throws Exception;

    @SelectProvider(type = MenuProvider.class, method = "getRoleMenuList")
    List<Long> getRoleMenuList(@Param("roleId") long roleId) throws Exception;

    @SelectProvider(type = MenuProvider.class, method = "getMenuById")
    MenuEntity getMenuById(@Param("menuID") long menuID) throws Exception;

    @SelectProvider(type = MenuProvider.class, method = "getMenuByParentId")
    List<MenuEntity> getMenuByParentId(@Param("parentID") long parentID) throws Exception;

    @SelectProvider(type = MenuProvider.class, method = "getMenusByAdminId")
    List<MenuEntity> getMenusByAdminId(@Param("adminId") Long adminId) throws Exception;
